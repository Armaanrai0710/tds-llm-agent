/**
 * LlmAgent - Advanced AI Assistant
 * v2.8.0 (Updated: AI Pipe integration + bug fixes)
 * Author: Armaan Singh Rai
 */
class LlmAgent {
    constructor() {
        this.version = '2.8.0';
    ...
    async init() {
        try {
            ...
            console.log(`LlmAgent v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize LlmAgent:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'LlmAgent', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to LlmAgent! How can I assist you?');
        }
    }
